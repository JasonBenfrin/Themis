...
module.exports = {
  ...
  async execute(member){
    if(member.guild.id != '880856257991409704') return
		...
		const background = await Canvas.loadImage('./images/mount_wallpaper.jpg')
		...
		context.closePath()
		context.clip()
		const avatar = await Canvas.loadImage(member.user.displayAvatarURL({size: 600, format: 'jpg'}))
